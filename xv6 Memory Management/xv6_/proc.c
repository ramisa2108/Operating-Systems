...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  // copy swapfile, physical and swap page metadata from parent
  if(curproc->pid > 2)
  {
...
exit(void)
{
  ...
  // remove swapfile and clear page metadata
  if(curproc->pid > 2)
  {
...
procdump(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    // iterate upto NPDENTRIES/2. upper addresses are for kernel
